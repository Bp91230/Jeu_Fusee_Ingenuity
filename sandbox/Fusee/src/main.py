...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, Fusee)
...
# inputs
def Set_ID_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Fusee)
        agent_object.Set_IDI = value
        # add code here if needed
        agent_object.ID_fuseeO(value)
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.definition_set_description("""Agent fusee """)
    ...
    igs.input_create("Set_ID", igs.STRING_T, None)

    igs.output_create("keyPressed", igs.IMPULSION_T, None)
    igs.output_create("ID_fusee", igs.STRING_T, None)

    igs.observe_input("Set_ID", Set_ID_input_callback, agent)
