...
# inputs
def keyPressed_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Moteur)
        agent_object.keyPressed()
    except:
        print(traceback.format_exc())

def ID_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Moteur)
        agent_object.IDI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def CoordPlanet_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Moteur)
        agent_object.CoordPlanetI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.input_create("keyPressed", igs.IMPULSION_T, None)
    igs.input_create("ID", igs.INTEGER_T, None)
    igs.input_create("CoordPlanet", igs.DATA_T, None)

    igs.output_create("setID", igs.STRING_T, None)

    igs.observe_input("keyPressed", keyPressed_input_callback, agent)
    igs.observe_input("ID", ID_input_callback, agent)
    igs.observe_input("CoordPlanet", CoordPlanet_input_callback, agent)
